...
service /students on new http:Listener(8080) {
    ...
    // function to remove student from table
    function remove(string[] studentDetail, int studentnumber) returns json {
        io:print("Handles remove request to all students");
        return all_students;
    } 
    // function to filter student from table
    function filter(string[] studentnumber, function(string) returns (boolean) func) returns json {
        io:print("Handle filter request");
        return all_students; 
    }
}
